...
ex.observers.append(FileStorageObserver('./gp_logs/'))
...
def ex_main(_run, fill_missing_, scale_type, window_size_, train_model, eval_model, subsample, stride, num_leaves_, learning_rate_, n_estimators_):
    ...
    #     print ("Data shape", X_train_df.shape, X_val_df.shape)
        ...
        c_X_train = X_train[~ (y_train==0)]
        c_y_train = y_train[~ (y_train==0)]
            
        print ("Training for faulty classes", c_X_train.shape)
            
        step1_model = train_model_for_class(c_X_train, c_y_train)
        step1_model_pred = step1_model.predict_proba(X_train)

        print ("Step1_Predictions:", step1_model_pred.shape)            

        X_train_predictions = step1_model_pred
...
#             GaussianProcessClassifier(kernel=kernel1+kernel2)
        ...
        step1_val_pred = step1_model.predict_proba(X_val)
        X_val_predictions = step1_val_pred
            
        step2_pred = fusion_model.predict(X_val_predictions)

        acc_val = accuracy_score(y_val, step2_pred)
        f1_val = f1_score(y_val, step2_pred, average='weighted')
        mcc_val = matthews_corrcoef(y_val, step2_pred)
        ...
        cm = confusion_matrix(y_val, step2_pred, labels=sorted(train_df['class'].unique()))
...
#                    matthews_corrcoef(y_val[y_val == c], model.predict(X_val[y_val == c])))
    ...
    results_df.to_excel("GP_v2_RobustScaler.xlsx")    
